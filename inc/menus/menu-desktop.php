...
$wcr_is_user_logged_in = $wcr_user_manager->is_user_logged_in();
$wcr_user_role         = $wcr_user_manager->get_user_role();
if ( ! $wcr_is_user_logged_in ) {
	$wcr_user_role = 'logged_out';
}
?>
<div class="mdc-top-app-bar__action-items desktop-menu-items">
	...
	foreach ( $menu_links as $link ) {
		...
		if (
			empty( $link['roles'] )
			|| in_array( 'all', $link['roles'] )
		) {
			$allowed = true;
		} elseif ( ! $wcr_is_user_logged_in && in_array( 'logged_out', $link['roles'] ) ) {
			$allowed = true;
		} elseif ( in_array( $wcr_user_role, $link['roles'] ) ) {
			$allowed = true;
		}
		...
		if ( $is_english )  {
			...
			?>
			<a href="<?php echo esc_url($link['url']); ?>" class="<?php echo sanitize_title($label) . '-button'; ?> mdc-top-app-bar__action-item mdc-button mdc-button--icon-leading <?php echo $is_sign_up ? 'sign-up-class' : ''; ?>">
				<?php if (!empty($link['icon_image'])) :
					$icon = $link['icon_image']; ?>
					<img src="<?php echo esc_url($icon['url']); ?>"
						alt="<?php echo esc_attr($icon['alt']); ?>"
